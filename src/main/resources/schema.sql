-- GlobeChat Database Schema
-- Initialized via spring.sql.init on application startup
...
CREATE TABLE IF NOT EXISTS moodsics (
    id BIGSERIAL PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_moodsics_uploaded_by ON moodsics(uploaded_by_id);
CREATE INDEX IF NOT EXISTS idx_moodsics_play_count ON moodsics(play_count DESC);
CREATE INDEX IF NOT EXISTS idx_moodsics_public ON moodsics(is_public) WHERE is_public = true;
...
CREATE TABLE IF NOT EXISTS chat_rooms (
    ...
    moodsic_paused BOOLEAN NOT NULL DEFAULT false,
    created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT CURRENT_TIMESTAMP
);
...
CREATE INDEX IF NOT EXISTS idx_chat_rooms_moodsic ON chat_rooms(current_moodsic_id);
CREATE INDEX IF NOT EXISTS idx_chat_rooms_coordinates ON chat_rooms(latitude, longitude);
...
CREATE INDEX IF NOT EXISTS idx_room_bans_room ON room_bans(chat_room_id);
CREATE INDEX IF NOT EXISTS idx_room_bans_user ON room_bans(banned_user_id);
